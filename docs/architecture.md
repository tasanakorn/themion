...
- **Coordination channels stay distinct** — prompt guidance teaches agents to use direct answers for simple requests, self-notes for their own tracked work, durable board notes for delegated work that must be completed or reported later, and volatile inbox messages for short coordination only. Involving another local agent requires explicit user authorization for delegation, parallel agent work, or another agent's help/review; task size, depth, or research wording alone does not authorize delegation. Multi-agent coordination should have one authoritative state owner, stable identifiers, explicit response channels, meaningful state updates, and a clear final outcome.
...
tools.rs
  ...
  │    ├─ fs_read_file, fs_write_file, fs_patch, fs_list_directory  (ignore ctx except project_dir for patch target resolution)
...
The TUI now also distinguishes persistent config changes from session-only runtime overrides. `/config profile use <name>` still switches profile and saves that choice to config, while `/session profile use <name>`, `/session profile set model=<value>`, and `/session profile set effort=<value>` reconfigure the live interactive agent for the current session only without rewriting config on disk. These live profile/config changes keep the same interactive `session_id`, transcript/history continuity, and runtime-owned session state; when the agent is busy, the new runtime settings defer to the next turn instead of tearing down the in-flight turn. `/session profile show` reports configured versus effective runtime state, and `/session profile reset` clears temporary session-only overrides. Persistent profile management also includes `/config profile clone <source> <dest>`, `/config profile delete <name>`, and `/config profile set effort=<value>`, with deletion blocked for the active profile and the `default` profile.
