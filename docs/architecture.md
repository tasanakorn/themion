...
The TUI now also distinguishes persistent config changes from session-only runtime overrides. `/config profile use <name>` still switches profile and saves that choice to config, while `/session profile use <name>` and `/session profile set model=<value>` rebuild the live interactive agent for the current session only without rewriting config on disk. `/session profile show` reports configured versus effective runtime state, and `/session profile reset` clears temporary session-only overrides. Persistent profile management also includes `/config profile clone <source> <dest>` and `/config profile delete <name>`, with deletion blocked for the active profile and the `default` profile.
