...
- **Coordination channels stay distinct** — prompt guidance teaches agents to use direct answers for simple requests, self-notes for their own tracked work, durable board notes for delegated work that must be completed or reported later, volatile inbox messages for short coordination only, and task requests when a remote task id plus status/result polling is needed. Multi-agent coordination should have one authoritative state owner, stable identifiers, explicit response channels, meaningful state updates, and a clear final outcome.
