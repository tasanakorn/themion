...
- `fs_patch` accepts unified-diff patch text plus optional `reason` and is the primary tool for modifying existing UTF-8 text files
- the `fs_patch` tool description explicitly asks for standard unified diff headers such as `--- a/path`, `+++ b/path`, and `@@` hunks, and warns not to use `*** Begin Patch` / `*** Update File` wrappers
- `fs_patch` supports raw standard unified diffs and markdown ```diff fences, validates project-relative header paths, rejects absolute paths plus `..` traversal outside the project directory, rejects create/delete/rename/conflict-marker input, and applies the whole request atomically in strict exact mode
...
- `fs_write_file` still supports direct text writes through `mode=raw`, but its tool description and app-level prompt guidance reserve it for creating new files rather than normal modification of existing text files
- `shell_run_command` accepts optional `result_limit` and `timeout_ms`, defaulting to `16384` bytes and `300000` ms
- Themion's default prompt and tool guidance do not recommend shell-based file mutation as the normal edit path; prefer `fs_patch` first and use shell mutation only when it is the only practical way to complete the change cleanly
