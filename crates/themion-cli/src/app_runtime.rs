...
#[cfg(feature = "stylos")]
pub(crate) type StylosSnapshotFuture =
    std::pin::Pin<Box<dyn std::future::Future<Output = StylosStatusSnapshot> + Send>>;
...
#[derive(Debug)]
pub(crate) struct SourceAnalysisRequest {
...
pub(crate) fn build_source_analysis_tool_invoker(
    app_tx: mpsc::UnboundedSender<AppEvent>,
) -> themion_core::tools::SourceAnalysisToolInvoker {
    std::sync::Arc::new(move |name: String, args: serde_json::Value| {
        ...
        > = Box::pin(async move {
            ...
            app_tx
                .send(AppEvent::SourceAnalysis(SourceAnalysisRequest {
                    action: name,
                    args,
                    reply_tx,
                }))
                .map_err(|_| anyhow::anyhow!("source analysis queue unavailable"))?;
            reply_rx
                .await
                .map_err(|_| anyhow::anyhow!("source analysis reply unavailable"))?
        });
        fut
    })
}
        ...
        if let Some(stylos) = stylos {
            refresh_stylos_status_snapshot(
                ...
                StylosAppStatusRefreshState {
                    ...
                    app_version: stylos.app_version,
                    app_version_hash: stylos.app_version_hash,
                    app_version_dirty: stylos.app_version_dirty,
...
pub(crate) fn build_main_agent(
    ...
    crate::app_state::build_agent(
        ...
        #[cfg(feature = "stylos")]
        local_agent_id,
        Some(build_local_agent_tool_invoker(local_agent_mgmt_tx.clone())),
        Some(build_source_analysis_tool_invoker(local_agent_mgmt_tx)),
        system_inspection,
        api_log_enabled,
        "master",
        "master",
...
pub(crate) struct RuntimeCommandContext<'a> {
    ...
    #[cfg(feature = "stylos")]
    pub local_instance_id: Option<&'a str>,
...
pub(crate) fn build_system_inspection_snapshot(
    ...
    let mut provider = SystemInspectionProvider {
        ...
        auth_configured: Some(match session.provider.as_str() {
            "openai-codex" => crate::app_state::resolve_codex_auth(session)
                .ok()
                .flatten()
                .is_some(),
            _ => session
...
pub(crate) fn agent_has_role(handle: &crate::tui::AgentHandle, role: &str) -> bool {
    ...
    handle.roles.iter().any(|candidate| {
        #[cfg(feature = "stylos")]
        {
            let candidate = if candidate == "main" {
                "master"
            } else {
                candidate.as_str()
            };
...
pub(crate) fn build_local_agent_status_entries_local(
    agents: &[crate::tui::AgentHandle],
...
pub(crate) struct LocalAgentRuntimeContext<'a> {
    ...
    pub source_analysis_tool_invoker: Option<themion_core::tools::SourceAnalysisToolInvoker>,
    pub api_log_enabled: bool,
}
...
fn create_local_agent(
    ...
    if roles.iter().any(|r| r == "interactive")
        && ctx
            .roster
            .iter()
            .any(|entry| entry.roles.iter().any(|r| r == "interactive"))
    {
        anyhow::bail!("invalid agent roles: expected at most one interactive agent");
    }
    ...
    let agent = crate::app_state::build_agent(
        ...
        #[cfg(feature = "stylos")]
        ctx.local_instance_id,
        #[cfg(feature = "stylos")]
        &agent_id,
        ctx.local_agent_tool_invoker,
        ctx.source_analysis_tool_invoker,
        None,
        ctx.api_log_enabled,
        &agent_id,
        &label,
    ...
    ctx.agents
        .push(build_local_agent_handle(NewLocalAgentHandleParts {
            agent,
            session_id,
...
pub(crate) fn start_watchdog_task(
    background_domain: &DomainHandle,
    runtime_tx: mpsc::UnboundedSender<AppRuntimeEvent>,
    _runtime_state: Arc<WatchdogRuntimeState>,
) {
    background_domain.spawn(async move {
        let mut interval = tokio::time::interval(Duration::from_millis(250));
        loop {
            interval.tick().await;
            if runtime_tx.send(AppRuntimeEvent::WatchdogTick).is_err() {
                break;
            }
        }
    });
...
pub(crate) fn resolve_incoming_prompt_disposition(
    ...
    let log_text = if is_note {
        let note_identifier = stylos_note_display_identifier(&request.prompt);
        let column = stylos_note_header_value(&request.prompt, "column").unwrap_or("unknown");
        format!(
...
pub(crate) fn plan_completed_note_follow_up(
    db: &Arc<DbHandle>,
    remote: &crate::local_prompts::IncomingPromptRequest,
    ...
    let main_count = agent_sources
        ...
        .filter(|agent| {
            agent
                .roles
                .iter()
                .any(|r| normalize_primary_role(r) == "master")
        })
        .count();
    if main_count != 1 {
        anyhow::bail!(
    ...
    let agents = agent_sources
        ...
        .map(|agent| {
            ...
            crate::stylos::StylosAgentStatusSnapshot {
                ...
                app_version_hash: agent.app_version_hash,
                app_version_dirty: agent.app_version_dirty,
    ...
    let agent_sources: Vec<AgentStatusSource> = input
        ...
        .map(|(idx, agent)| {
            ...
            AgentStatusSource {
                ...
                app_version_hash: input.app_version_hash.clone(),
                app_version_dirty: input.app_version_dirty,
...
pub(crate) fn refresh_stylos_status_snapshot(
    ...
    build_and_publish_stylos_status_snapshot(
        ...
        StylosAppStatusSnapshotInput {
            ...
            app_version_hash: state.app_version_hash.to_string(),
            app_version_dirty: state.app_version_dirty,
            rate_limits: state.rate_limits.cloned(),
...
pub(crate) fn build_and_publish_stylos_status_snapshot(
    ...
    let snapshot = build_stylos_status_snapshot_from_runtime(StylosStatusRefreshInput {
        ...
        app_version_hash: input.app_version_hash,
        app_version_dirty: input.app_version_dirty,
