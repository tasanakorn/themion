...
use crate::client::{ChatBackend, Message, ResponseMessage, ToolCall, FunctionCall, Usage, UsageDetails};
        ...
        match msg.role.as_str() {
            ...
            "assistant" => {
                if let Some(ref tcs) = msg.tool_calls {
                    // Prepend output_text if content non-empty
...
fn translate_tools(tools: &Value) -> Value {
    ...
    let translated: Vec<Value> = arr.iter().map(|t| {
        if t["type"] == "function" {
            let f = &t["function"];
            serde_json::json!({
                "type": "function",
...
impl ChatBackend for CodexClient {
    async fn chat_completion_stream(
        ...
        // tool call slots keyed by item_id (string from SSE)
        let mut tool_slots: std::collections::HashMap<String, ToolCallSlot> = std::collections::HashMap::new();
                    ...
                    if let (Some(event_type), Some(data_str)) = (current_event.take(), current_data.take()) {
                        ...
                        match event_type.as_str() {
                            ...
                            "response.function_call_arguments.delta" => {
                                if let (Some(item_id), Some(delta)) = (data["item_id"].as_str(), data["delta"].as_str()) {
                            ...
                            "response.output_item.added" => {
                                ...
                                if item["type"] == "function_call" {
                                    ...
                                    tool_slots.insert(item_id, ToolCallSlot {
                                        id: call_id,
                                        name,
                            ...
                            "response.failed" => {
                                let err_msg = data["response"]["error"]["message"].as_str()
        ...
        let tool_calls = if tool_slots.is_empty() {
            ...
            let mut calls: Vec<ToolCall> = tool_slots.into_values().map(|slot| ToolCall {
                id: slot.id,
