...
pub async fn open_session(cfg: &StylosConfig, overrides: &SessionOverrides) -> Result<zenoh::Session> {
    ...
    config.insert_json5("listen/endpoints", &jq_arr(&listen))
    ...
    config.insert_json5("connect/endpoints", &jq_arr(&connect))
        .map_err(|e| StylosError::Transport(format!("set connect: {e:?}")))?;

    // Scouting.
    if let Some(sc) = &cfg.zenoh.scouting {
        if let Some(mc) = &sc.multicast {
            soft_set(&mut config, "scouting/multicast/enabled", &mc.enabled.to_string());
            soft_set(&mut config, "scouting/multicast/address", &jq(&mc.address));
            soft_set(&mut config, "scouting/multicast/interface", &jq(&mc.interface));
            if let Some(ac) = &mc.autoconnect {
                soft_set(&mut config, "scouting/multicast/autoconnect/peer",   &jq(&ac.peer));
                soft_set(&mut config, "scouting/multicast/autoconnect/router", &jq(&ac.router));
            }
        }
        if let Some(g) = &sc.gossip {
            soft_set(&mut config, "scouting/gossip/enabled", &g.enabled.to_string());
        }
    } else {
...
pub async fn log_session_info(session: &zenoh::Session) {
    ...
    eprintln!("stylos session zid={zid} peers={} routers={}", peers.len(), routers.len());
}
